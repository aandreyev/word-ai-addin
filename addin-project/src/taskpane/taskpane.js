...
async function getAIAnalysis() {
  try {
    ...
    console.log(
      "🔍 getAIAnalysis: Stored paragraph mapping with",
      currentParagraphMapping.length,
      "items"
    );
...
async function applySuggestions() {
  ...
  try {
    ...
    const appliedCount = await window.aiDocumentReviewService.applySuggestions(
      currentSuggestions,
      currentParagraphMapping
    );
  ...
  try {
    ...
    if (window.aiDocumentReviewService && window.aiDocumentReviewService.aiService) {
      ...
      if (aiService.getApiKey) {
        ...
        console.log(
          "🔑 Detected API key:",
          detectedKey ? detectedKey.substring(0, 8) + "..." : "none"
        );
      ...
      if (
        ...
        process.env.GEMINI_API_KEY !== "API_KEY_NOT_SET"
      ) {
